                    ...
                    <div x-show="open" x-collapse class="space-y-0.5 pb-1">
                        ...
                        <label class="flex items-center gap-2.5 cursor-pointer min-h-[28px] group pl-1">
                            <input type="checkbox" value="{{ $item }}"
                                   ...
                                   class="w-4 h-4 rounded text-blue-600 border-gray-300 shrink-0">
                            <span class="text-xs text-gray-600 group-hover:text-gray-900">{{ $item }}</span>
                        </label>
                        @endforeach
                    </div>
                </div>
...
function customerSearch() {
    return {
        ...
        startResize(event, col) {
            ...
            const onMove = (e) => {
                this.colWidths[col] = Math.max(60, startW + e.pageX - startX);
            };
            const onUp = () => {
        ...
        async search(resetPage = true) {
            ...
            const res = await fetch(`{{ route('customers.index') }}?${params}`, {
        ...
        clearFilters() {
            this.filters = { customer_id: '', store_ids: [], treatment_types: [], treatment_areas: [] };
